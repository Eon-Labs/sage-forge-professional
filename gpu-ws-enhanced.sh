...
# Connectivity check
check_connectivity() {
    log_info "Checking GPU workstation connectivity..."
    
    if ! ping -c 2 "$REMOTE_IP" >/dev/null 2>&1; then
        log_error "Cannot reach GPU workstation at $REMOTE_IP"
        log_info "Check ZeroTier connection: sudo zerotier-cli peers"
        exit 1
    fi
    ...
    log_success "GPU workstation connectivity confirmed"
}
