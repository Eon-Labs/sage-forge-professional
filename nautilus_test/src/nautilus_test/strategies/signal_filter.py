...
class SignalQualityFilter:
    ...
    def _evaluate_volatility_context(self, market_regime: str) -> float:
        ...
        # Volatility analysis could be added here if needed
    ...
    def _evaluate_support_resistance(self, current_price: float, signal_direction: str) -> float:
    ...
    def _evaluate_historical_performance(
        self, signal_direction: str, signal_strength: float, market_regime: str
    ) -> float:
        ...
        for signal in self.signal_history:
            if (
                ...
                and signal.market_context.get("regime") == market_regime
                and abs(signal.strength - signal_strength) < 0.2
            ):
    ...
    def _calculate_confidence(self, quality_score: float, market_regime: str) -> float:
        ...
        regime_adjustment = {"TRENDING": 1.1, "RANGING": 1.0, "VOLATILE": 0.7, "UNKNOWN": 0.8}.get(
            market_regime, 0.8
        )
    ...
    def _estimate_trade_metrics(
        self, signal_direction: str, current_price: float, quality_score: float
    ) -> tuple[float, float]:
        """Estimate expected profit and risk-reward ratio."""
        if len(self.price_history) < 20:
            return 0.0, 1.0
    ...
    def update_signal_performance(self, signal: TradingSignal, trade_result: str, pnl: float):
