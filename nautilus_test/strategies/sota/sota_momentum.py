...
class SOTAMomentum(Strategy):
    ...
    def __init__(self, config: SOTAMomentumConfig):
        ...
        console.print(
            "[cyan]📊 Advanced Features: Momentum Persistence, Volatility Breakouts, Multi-TF Confluence[/cyan]"
        )
    ...
    def _process_sota_signals(self, bar: Bar, current_bar: int):
        ...
        if not self.multitimeframe.validate_signal(
            signal_direction, list(self.prices)[-50:], list(self.returns)[-30:]
        ):
            return
    ...
    def _generate_sota_signal(self) -> tuple[str, float]:
        ...
        combined_signal = self._combine_signals(
            momentum_signal, volatility_signal, microstructure_signal
        )
    ...
    def _momentum_persistence_signal(self) -> tuple[str, float]:
        ...
        momentum_alignment = (short_momentum > 0 and medium_momentum > 0 and long_momentum > 0) or (
    ...
    def _combine_signals(
        self, momentum_sig, volatility_sig, microstructure_sig
    ) -> tuple[str, float]:
    ...
    def _execute_sota_trade(self, direction: str, strength: float, bar: Bar, current_bar: int):
        ...
        bar_timestamp = pd.Timestamp(bar.ts_event, unit="ns")
        console.print(
            f"[bold green]💰 SOTA Trade: {direction} {position_size:.4f} @ {bar.close} at {bar_timestamp} (strength: {strength:.2f})[/bold green]"
        )
    ...
    def _should_exit_position(self, current_price: float) -> bool:
        ...
        if self.unrealized_pnl > abs(
            self.entry_price * self.current_position_size * 0.005
        ):  # 0.5% profit
            return True

        # 5. Stop loss on adverse moves
        if self.unrealized_pnl < -abs(
            self.entry_price * self.current_position_size * 0.003
        ):  # 0.3% loss
            return True
    ...
    def _log_performance_metrics(self, current_bar: int):
        ...
        if self.total_signals > 0:
            ...
            console.print(
                f"[cyan]📊 Bar {current_bar}: {self.profitable_signals}/{self.total_signals} signals profitable ({win_rate:.1f}%)[/cyan]"
            )
    ...
    def on_stop(self):
        ...
        if self.total_signals > 0:
            ...
            console.print(
                f"[green]  💰 Profitable: {self.profitable_signals} ({win_rate:.1f}%)[/green]"
            )
...
class MomentumPersistenceDetector:
    ...
    def calculate_momentum_strength(self, returns: list[float]) -> float:
        ...
        return float(persistence * min(acceleration, 2.0) * abs(short_mom) * 50)
...
if __name__ == "__main__":
    console.print(
        "[bold blue]🚀 SOTA Momentum Strategy - NautilusTrader Native Implementation[/bold blue]"
    )
    console.print(
        "[cyan]Features: Momentum Persistence, Volatility Breakouts, Multi-TF Confluence, Adaptive Sizing[/cyan]"
    )
    console.print(
        "[yellow]⚠️  This is a NT-native strategy module - use with NT backtest runners[/yellow]"
    )
