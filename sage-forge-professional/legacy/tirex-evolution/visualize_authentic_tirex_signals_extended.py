...
def load_extended_market_data():
    """Load extended BTCUSDT market data from DSM."""
    try:
        from sage_forge.data.manager import ArrowDataManager
        
        console.print("📊 Loading EXTENDED market data from DSM...")
...
def generate_authentic_tirex_signals(tirex_model, market_data):
    ...
    try:
        ...
        stride = max(1, (available_data - min_context_window) // 100)
        sample_points = range(0, available_data - min_context_window, stride)
        
        console.print(f"📊 Context window: {min_context_window} bars, Sampling {len(sample_points)} points (stride={stride})")

        for i, start_idx in enumerate(sample_points):
            if i > 0 and i % 10 == 0:
                console.print(f"   Processing prediction {i+1}/{len(sample_points)}...")
            ...
            if prediction is not None and prediction.direction != 0:
                ...
                if signal_bar_index < len(market_data):
                    current_bar = market_data.iloc[signal_bar_index]
...
def plot_authentic_tirex_signals(df, authentic_signals):
    ...
    if buy_signals:
        ...
        for signal in buy_signals:
            ...
            # Position triangle below the exact bar
            low_price = bar_data['low']
            offset_price = low_price - (bar_data['high'] - low_price) * triangle_offset_ratio
    ...
    if sell_signals:
        ...
        for signal in sell_signals:
            ...
            # Position triangle above the exact bar
            high_price = bar_data['high']
            offset_price = high_price + (bar_data['high'] - bar_data['low']) * triangle_offset_ratio
    ...
    for signal in buy_signals + sell_signals:
        ...
        if signal['signal'] == 'BUY':
            ...
            text_price = bar_data['low'] - (bar_data['high'] - bar_data['low']) * label_offset_ratio
        else:
            # Label above triangle
            text_price = bar_data['high'] + (bar_data['high'] - bar_data['low']) * label_offset_ratio
...
def display_authentic_signal_analysis(authentic_signals):
    ...
    if authentic_signals:
        ...
        # Sort signals by timestamp for chronological order
        sorted_signals = sorted(authentic_signals, key=lambda x: x['timestamp'])
...
def main():
    ...
    try:
        ...
        market_data = load_extended_market_data()
